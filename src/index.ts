#!/usr/bin/env node
/**
 * Brex MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Brex API token as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Brex corporate cards, expenses, payments, budgets, vendors, and transactions through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Brex API token as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-brex-api-token>',
      howToGetKey:
        'Generate an API token at https://dashboard.brex.com/settings/developer',
    },
    configTemplate: {
      mcpServers: {
        brex: {
          ...
          headers: {
            Authorization: 'Bearer <your-brex-api-token>',
          },
        },
      },
    },
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/brex/',
    },
  });
});
...
<head>
  ...
  <title>Brex MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Brex MCP Server</h1>
    <p class="subtitle">Access Brex corporate cards, expenses, payments, budgets, vendors, and transactions through MCP tools.</p>

    <div class="card">
      <h2>How to get your Brex API token</h2>
      <ol class="steps">
        <li>Log in to <a href="https://dashboard.brex.com" target="_blank" rel="noopener">Brex Dashboard</a>.</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">Brex API Token</label>
...
function createMCPServer(client: BrexClient): Server {
  const server = new Server(
    { name: 'brex-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  const client = new BrexClient(apiKey);
...
<head>
  ...
  <title>Authorize \u2014 Brex MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Brex MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Brex MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Brex MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides Brex API token)`);
});
// OAuth upgrade 2026-03-19T00:52:11Z
