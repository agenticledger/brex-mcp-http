#!/usr/bin/env node
/**
 * Brex MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Brex secrets. It is a *client* of the Connections Broker
 ...
 * POST /token for provider=brex -> { accessToken }, call the Brex API
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: '2.0.0',
    description: 'Manage Brex expenses, cards, transactions, and budgets via MCP.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Brex access token as Bearer.',
      },
    },
    configTemplate: {
      mcpServers: {
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves Brex via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <brex-access-token>',
    ],
  });
});
...
function createMCPServer(resolveClient: ClientResolver): Server {
  const server = new Server(
    { name: 'brex-mcp-server', version: '2.0.0' },
    { capabilities: { tools: {} } }
  );
...
async function connectMessage(principal: string): Promise<string> {
    ...
    return JSON.stringify(
      {
        ...
        authKind: 'static',
        message:
          'Brex is not connected for this caller yet. This is an API-key provider: an admin must vault the key with the Connections Broker (POST /credential for this provider). Once vaulted, run the tool again — it will work.',
      },
      null,
      2
    );
  ...
  if ('error' in started) {
    return `Brex isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      status: 'connection_required',
      provider: brokerProvider,
      message:
        'Brex is not connected for this caller yet. Open the connect link below once (sign in and grant access), then run the tool again — it will work.',
...
app.post('/mcp', async (req, res) => {
  ...
  if (raw) {
    const client = new BrexClient(raw);
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { 'Authorization': 'Bearer <your-brex-access-token>' },
      });
      return;
    }
    ...
    resolveClient = async () => {
      ...
      if (tok.status === 'connected') {
        return { kind: 'client', client: new BrexClient(tok.accessToken) };
      }
...
app.listen(PORT, () => {
  console.log(`Brex MCP HTTP Server v2.0.0 (broker-first)`);
